...
const EditProduct = () => {
  ...
  const handleSubmit = async () => {
    ...
    try {
      const response = await axios.put(`${BACKEND_URL}/product/${id}`, formData);
      ...
      toast.error(error.response?.data?.message || 'Error updating product. Please try again.');
    ...
    >
      ...
      <MenuItem value="false">Inactive</MenuItem>
          ...
          <Grid item xs={12} sm={6}>
            ...
            {image && (
              <Box sx={{ position: 'relative', height: '200px', width: '200px', marginTop: 2 }}>
                <Cropper
                  image={image}
                  crop={crop}
                  zoom={zoom}
            ...
            {existingImage && (
              <img src={existingImage} alt="Existing" style={{ width: '100%', marginTop: '10px' }} />
            )}
          </Grid>

          <Grid item xs={12} sx={{ display: 'flex', justifyContent: 'flex-end', marginTop: 2 }}>
          <Button variant="contained" color="primary" onClick={handleSubmit} disabled={loading} sx={{ marginRight: 2 }}>
            {loading ? <CircularProgress size={24} /> : 'Update Product'}
          </Button>
          <Button variant="outlined" color="secondary" onClick={() => navigate('/dashboard/admin/products')}>
            Cancel
          </Button>
        </Grid>
        </Grid>
